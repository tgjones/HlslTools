...
QUAD_REAL4 ToCMY(QuadVertexOutput IN,
		    uniform sampler2D SceneSampler) : COLOR
{   
    QUAD_REAL4 rgba = tex2D(SceneSampler, IN.UV);
    return QUAD_REAL4(rgb2cmy(rgba.xyz),rgba.w); // don't lose alpha
}  
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, ToCMY(gSceneSampler) ) );
...
> {
	...
	VertexShader = compile vs_3_0 ScreenQuadVS2(QuadTexelOffsets);
		...
		CullMode = None;
	PixelShader = compile ps_3_0 ToCMY(gSceneSampler);
    }
}
