...
//  string Script = "Technique=Technique?Simple:Simple10:Textured:Textured10:SimpleQuadratic:SimpleQuadratic10:TexturedQuadratic:TexturedQuadratic10;";
> = 0.8;
...
float4 plasticPS(vertexOutput IN,
    ...
    float3 diffContrib;
    float3 specContrib;
	plastic_shared(IN,Kd,Ks,SpecExpon,LampColor,AmbiColor,diffContrib,specContrib);
...
float4 plasticQPS(vertexOutput IN,
    ...
    plastic_shared(IN,Kd,Ks,SpecExpon,Cl,AmbiColor,diffContrib,specContrib);
    float3 result = specContrib + (SurfaceColor * diffContrib);
    return float4(result,1);
}
...
technique10 TexturedQuadratic10 <
	string Script = "Pass=p0;";
> {
    pass p0 <
	string Script = "Draw=geometry;";
    > {
        SetVertexShader( CompileShader( vs_4_0, std_VS(gWorldITXf,gWorldXf,
				...
				gLamp0Pos) ) );
        SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_4_0, plasticQPS_t(gSurfaceColor,gColorSampler,
						gKd,gKs,gSpecExpon,
						gLamp0Color,gLamp0Intensity,
						gAmbiColor) ) );
	    SetRasterizerState(DisableCulling);
...
> {
    ...
    > {
		...
		AlphaBlendEnable = false;
		CullMode = None;
        PixelShader = compile ps_3_0 plasticQPS_t(gSurfaceColor,gColorSampler,
						gKd,gKs,gSpecExpon,
						gLamp0Color,gLamp0Intensity,
						gAmbiColor);
    }
