...
QUAD_REAL4 ToCMYK(QuadVertexOutput IN,
		    uniform sampler2D SceneSampler) : COLOR
{   
    QUAD_REAL3 rgb = tex2D(SceneSampler, IN.UV).xyz; // CMYK has no alpha
    return rgb2cmyk(rgb);
}  
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, ToCMYK(gSceneSampler) ) );
...
> {
	...
	VertexShader = compile vs_3_0 ScreenQuadVS2(QuadTexelOffsets);
		...
		CullMode = None;
	PixelShader = compile ps_3_0 ToCMYK(gSceneSampler);
    }
}
