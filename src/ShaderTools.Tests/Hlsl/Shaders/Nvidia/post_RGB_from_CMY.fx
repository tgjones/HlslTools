...
QUAD_REAL4 FromCMY(QuadVertexOutput IN,
		    uniform sampler2D SceneSampler) : COLOR {   
    QUAD_REAL4 cmy = tex2D(SceneSampler, IN.UV);
    return QUAD_REAL4(cmy2rgb(cmy.xyz),cmy.w); // we don't lose alpha....
}  
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, FromCMY(gSceneSampler) ) );
...
> {
	...
	VertexShader = compile vs_3_0 ScreenQuadVS2(QuadTexelOffsets);
		...
		CullMode = None;
	PixelShader = compile ps_3_0 FromCMY(gSceneSampler);
    }
}
