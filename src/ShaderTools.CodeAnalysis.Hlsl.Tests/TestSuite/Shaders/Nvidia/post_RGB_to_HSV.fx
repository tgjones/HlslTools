...
QUAD_REAL4 ToHSV(QuadVertexOutput IN,
		    uniform sampler2D SceneSampler) : COLOR
{   
    QUAD_REAL4 rgba = tex2D(SceneSampler, IN.UV);
    return QUAD_REAL4(rgb_to_hsv(rgba.xyz),rgba.w); // don't lose alpha
}  
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, ToHSV(gSceneSampler) ) );
...
> {
	...
	VertexShader = compile vs_3_0 ScreenQuadVS2(QuadTexelOffsets);
		...
		CullMode = None;
	PixelShader = compile ps_3_0 ToHSV(gSceneSampler);
    }
}
