...
QUAD_REAL4 FromCMYK(QuadVertexOutput IN,
		    uniform sampler2D SceneSampler) : COLOR
{   
    QUAD_REAL4 cmyk = tex2D(SceneSampler, IN.UV);
    return QUAD_REAL4(cmyk2rgb(cmyk),1); // cmyk has no alpha....
}  
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, FromCMYK(gSceneSampler) ) );
...
> {
	...
	VertexShader = compile vs_3_0 ScreenQuadVS2(QuadTexelOffsets);
		...
		CullMode = None;
	PixelShader = compile ps_3_0 FromCMYK(gSceneSampler);
    }
}
